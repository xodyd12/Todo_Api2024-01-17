...
public class UserService {
    ...
    public UserSignUpResponseDTO create(UserSignUpRequestDTO dto, String profilePath) {
        ...
        if (userRepository.existsByEmail(email)) {
            ...
            throw new DuplicatedEmailExcetion("중복된 이메일입니다!!");
        }
    ...
    public LoginResponseDTO promoteToPremium(TokenUserInfo userInfo) {
        ...
        if (user.getRole() != Role.COMMON) {
            throw new IllegalStateException("일반 회원이 아니면 승격할 수 없습니다.");
        }
    ...
    /**
     *  업로드한 프필 사진을 서버에 저장하고 저장된 경로를 리턴
     * @param originalFile - 업로드된 파일의 정보객체
     * @return 실제 이미지가 저장된 서버의 디렉터리 경로
     */
